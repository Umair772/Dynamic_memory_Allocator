...
void* mm_malloc(size_t size) {
  ...
  if (!(blockSize < reqSize + MIN_BLOCK_SIZE)) {
    ...
    BlockInfo* split = (BlockInfo*) UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize);
    split->sizeAndTags = (blockSize - reqSize) | TAG_PRECEDING_USED;
    *((size_t*) UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize - WORD_SIZE)) = split->sizeAndTags;
    ...
    next->sizeAndTags = next->sizeAndTags | TAG_PRECEDING_USED;
  }
...
void mm_free(void *ptr) {
  ...
  followingBlock->sizeAndTags = (size_t)followingBlock->sizeAndTags & (~TAG_PRECEDING_USED);
  blockInfo->sizeAndTags = blockInfo->sizeAndTags & (~TAG_USED);
  payloadSize -=  WORD_SIZE;
